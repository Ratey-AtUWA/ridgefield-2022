...
with(rf_boundary, lines(Easting, Northing, col="#00000040", lwd=3))
contour(x = seq(min(elev.TEMP[,1]), max(elev.TEMP[,1]), l = 50),
      y = seq(min(elev.TEMP[,2]), max(elev.TEMP[,2]), l = 50), 
      z = matrix(elev.TEMP$Elevation,nrow=50, byrow = T),
      col="#FFE08080", labcex = 0.8, add = TRUE)
image(x = seq(min(elev.TEMP[,1]), max(elev.TEMP[,1]), l = 50),
      y = seq(min(elev.TEMP[,2]), max(elev.TEMP[,2]), l = 50),
      z = matrix(elev.TEMP$Elevation,nrow=50, byrow = T),
      col=viridis(64, alpha = 0.33), add = TRUE)
#
# end code