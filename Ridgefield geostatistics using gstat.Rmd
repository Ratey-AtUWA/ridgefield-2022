---
title: "Ridgefield to 2019 spatial analyses"
output: 
...
rf18a$Year <- as.factor(rf18a$Year)
rf_border <- read.csv(paste0(git,"rf_hull.csv"))
rf18a <- droplevels(rf18a[-which(rf18a$Depth_upper_cm > 1 & 
                                           rf18a$Type=="Soil"),])
```
...
extent_UTM <- st_transform(extent, crs=UTM50)
...
rftiles <- get_tiles(extent_UTM, provider = "Esri.WorldImagery", 
                     crop = TRUE, zoom = 13) # make map object
...
mtext(side=2, line=2, text="Northing (UTM Zone 50, m)",
      font=2, cex=1.2)
with(rf18a, points(Easting, Northing, pch = 3, cex = 0.75, col = "gold"))
# with(rf_border, lines(Easting,Northing, lty = 3, col = "darkgreen"))
polygon(rf_boundary,lty=c("12"),lwd=2,border="#FFFFFFC0")
...
# Calculate global Moran's I
data_temp <- na.omit(rf18a[,c("Easting", "Northing", var0)])
...
```{r local-Moran, message=FALSE, warning=FALSE, fig.height=7.15, fig.width=10, fig.align='center', out.width="80%", fig.cap="Map of Local Moran's I for TOC concentrations in Ridgefield soils. The Global Moran's I parameter is also shown beneath the legend.", results='hide', echo=FALSE}
...
legend("top", bty = "o", cex = 1.5, title.cex = 1.5,
       ...
       text.col = "white", title.col = "pink", y.intersp = 0.9, bg="#384838")
...
```{r lisa-plot, message=FALSE, warning=FALSE, fig.height=7.15, fig.width=10, fig.align='center', out.width="80%", fig.cap="Map of LISA for C.pct concentrations in the Ridgefield soil data.", results='hide',echo=FALSE}
data_temp <- na.omit(as.data.frame(cbind(rf18a$Easting,rf18a$Northing,
                                         rf18a$C.pct)))
...
```{r multi-LISA, fig.height=10, fig.width=7.8, fig.align='center', fig.cap="LISA autocorrelation maps for C.pct, Al, Fe, S, As, Cu, Pb, and Zn concentrations in the Ridgefield soil data, 2019-2022.", message=FALSE, warning=FALSE, echo=FALSE}
par(mfrow=c(3,2), mar=c(0,0,0,0), oma=c(3,3,0.5,0.5), mgp=c(1.6,0.2,0), 
    tcl=0.3,lend="square", ljoin="mitre")
palette(c("black","#FF8080","#8080FF","orange","skyblue",
          "#B00000","#0000B0","#FFA50040","#87CEEB40","white"))
varz <- c("Gravel.pct","C.N.ratio","pH.CaCl2","EC","CEC","Na_ex")
...
data0 <- na.omit(rf18a[,c("Easting","Northing","C.pct")])
...
```{r spplot-spdf, fig.height=5, fig.width=8, fig.align='center', out.width="80%", fig.cap="Visualization of spatial point data object for log10-transformed C.pct concentrations in Ridgefield soils 2019-2022.", message=FALSE, warning=FALSE, results='hold', echo=FALSE}
...
```{r TOC-tukey-plot, echo=2:19, fig.width=8, fig.height=7.05, fig.align='center', fig.cap="Map of C.pct concentrations expressed as symbols for concentration ranges (UQR is 75th percentile, upWsk is upper whisker, med is median, LQR is 25th percentile, loWsk is lower whisker). Data are from Ridgefield soils 2019-2022.", out.width="80%", message=FALSE, warning=FALSE,echo=FALSE}
...
legend(494500, 6407050, legend = rev(c("-outliers","loWsk to LQR","LQR to med",
...
**Bins for boxplot-categorised-point map in Figure \@ref(fig:TOC-tukey-plot):**

```{r tukey-map-bins, echo=FALSE}
tukbins <- data.frame(bins=c("-outliers","loWsk to LQR","LQR to med",
                                "med to UQR","UQR to upWsk","+outliers"),
      from=signif(c(-4,boxplot.stats(C.pct_sp@data[,1])$stats),3),
      to=signif(c(boxplot.stats(C.pct_sp@data[,1])$stats,Inf),3))
knitr::kable(tukbins)
```
...
```{r make variogram object, message=FALSE, warning=FALSE, echo=FALSE, fig.align='center', fig.cap="Plot of experimental binned variogram for C.pct in Ridgefield soils 2019-2022.", fig.height=3.5, fig.width=4.5, out.width="50%"}
data0 <- na.omit(rf18a[,c("Easting","Northing","Gravel.pct")])
data0 <- data0[-which(data0$Gravel.pct<1e-4),]
data0[,3] <- log10(data0[,3])
...
vgr <- variog_Gravel.pct; vgr[,2] <- round(vgr[,2],1); vgr[,3] <- round(vgr[,3],3)
vgr[,1:3]; rm(vgr)
```
...
```{r plot-exp-model-variograms, fig.height=3.6, fig.width=5, fig.align='center', fig.cap="Plot of experimental binned variogram, and exponential variogram model, for Gravel.pct in Ridgefield soils 2019-2022.", out.width="50%", results='hide', echo=FALSE}
...
text(600,0.025,pos=2,offset=0, cex=0.75, family='mono', col = 2,
     labels=paste(
...
kriged_Gravel.pct <- krige(formula = Gravel.pct~1, locations = Gravel.pct_sp, 
                      newdata = afgrid, model = vmodel_Gravel.pct)
...
```{r spplot-kriging-output, fig.width=8, fig.height=5.85, fig.align='center', echo=2, fig.cap="Plots of simple kriging predictions and variance for log-transformed Gravel.pct in Ridgefield soils 2019-2022.", out.width="75%", warning=FALSE, error=FALSE, results='hold', echo=FALSE}
par(oma=c(0,0,0,0))
...
```{r map-krig-mod-pred, fig.height=12.8, fig.width=7.8, fig.align='center', fig.cap="Map showing (a) predictions and (b) variance for kriging model of Gravel.pct in Ridgefield soils. Sample points are tiny dots.", message=FALSE, warning=FALSE, echo=FALSE}
...
quilt.plot(afgrid@coords[,1], afgrid@coords[,2],kriged_Gravel.pct@data[,1],
           add = T, horizontal=T, 
...
points(rf18a[,c("Easting","Northing")], pch=16, cex=0.1, col = "gold")
...
rect(494500, 6402250, 501000, 6403000, col="#FFFFFF80", border=NA)
quilt.plot(afgrid@coords[,1], afgrid@coords[,2],kriged_Gravel.pct@data[,2],
           add = T, horizontal=T, 
           col = colorRampPalette(colgrad, alpha = TRUE)(64),
...
addnortharrow(border=1, lwd=1, text.col=1, padin=c(0.1,0.1), scale = 0.9)
addscalebar(plotepsg = 32750, pos = "topleft")
points(rf18a[,c("Easting","Northing")], pch=16, cex=0.1, col="orchid")
