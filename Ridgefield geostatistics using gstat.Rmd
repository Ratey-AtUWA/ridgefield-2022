...
```{r local-Moran, message=FALSE, warning=FALSE, fig.height=7.15, fig.width=10, fig.align='center', out.width="80%", fig.cap="Map of Local Moran's I for TOC concentrations in Ridgefield soils. The Global Moran's I parameter is also shown beneath the legend.", results='hide'}
...
```{r spplot-spdf, fig.height=5, fig.width=8, fig.align='center', out.width="80%", fig.cap="Visualization of spatial point data object for log10-transformed C.pct concentrations in Ridgefield soils 2019-2022.", message=FALSE, warning=FALSE, results='hold'}
...
```{r make-variog-obj, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Plot of experimental binned variogram for C.pct in Ridgefield soils 2019-2022.", fig.height=3.5, fig.width=4.5, out.width="50%"}
data0 <- na.omit(rf18a[,c("Easting","Northing","Gravel.pct")])
data0 <- data0[-which(data0$Gravel.pct<1e-4),] # find and remove zeros
data0[,3] <- log10(data0[,3])
...
**Data for Figure \@ref(fig:make-variog-obj)**: `np` is number of points,
`gamma` (&gamma;) is semivariance.

```{r print-variog-obj, paged.print=FALSE, echo=FALSE}
vgr <- variog_Gravel.pct
vgr[,2] <- round(vgr[,2],1)
vgr[,3] <- round(vgr[,3],3)
flextable(vgr[,1:3]); rm(vgr)
```
...
```{r spplot-kriging-output, fig.width=8, fig.height=5.85, fig.align='center', echo=2, fig.cap="Plots of simple kriging predictions and variance for log-transformed Gravel.pct in Ridgefield soils 2019-2022.", out.width="75%", warning=FALSE, error=FALSE, results='hold'}
par(oma=c(0,0,0,0))
...
```{r map-krig-mod-pred, fig.height=12.8, fig.width=7.8, fig.align='center', fig.cap="Map showing (a) predictions and (b) variance for kriging model of Gravel.pct in Ridgefield soils. Sample points are tiny dots.", message=FALSE, warning=FALSE}
